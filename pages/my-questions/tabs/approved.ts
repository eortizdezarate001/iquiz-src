...
@IonicPage()
@Component({
  selector: 'page-approved-questions',
  template: `
    <ion-header>
      <ion-navbar hideBackButton>
        ...
        <ion-title>Approved questions</ion-title>
      </ion-navbar>
    </ion-header>
...
export class ApprovedQuestions {
  ...
  getQuestions() {
    this.storage.get('loginUsername').then((user) => {
      this.http.get('http://iquiz.x10.bz/get-question.php?key=myQuestions-approved&username=' + user)
      .map(res => res.json())
      .subscribe(data => {
        if(data.length !== 0){
